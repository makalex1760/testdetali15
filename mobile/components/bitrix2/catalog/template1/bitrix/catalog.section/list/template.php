...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
	...
	?>	
		<div class="col2_aopen_list">
		<div class="bx_catalog_item_title"><a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" title="<? echo $productTitle; ?>"><? echo $productTitle; ?></a></div>
		<div class="bx_catalog_item_articul">
		<table class="prop_aopen_list">
		<tr>
<?  		
			foreach ($arItem['DISPLAY_PROPERTIES'] as $arOneProp)
			{ 
			switch($arOneProp['DISPLAY_VALUE']){
												case 'нет в наличии': $col='red';break;
												case 'в наличии': $col='green';break;
												case 'на складе': $col='yel';break;
												default: $col='green';break;
												}
			
			if($arOneProp['CODE']=='NALICHIE') { 	
			
													$prop_name="<li>".$arOneProp['DISPLAY_VALUE']."</li>";
													$prop_value=""; 
													$gr_nal="<span class='nal $col'>";
													$obspan="</span>";
													}else
													{
													$prop_name=$arOneProp['NAME'];	
													$prop_value=$arOneProp['DISPLAY_VALUE'];
													$gr_nal="<li/>";$obspan=":&nbsp;";
													}	
													
				?>	<? if($arOneProp['CODE']=='NALICHIE' || $arOneProp['CODE']=='CML2_MANUFACTURER' || $arOneProp['CODE']=='CML2_ARTICLE' ): ?>
					<td ><? echo $gr_nal; echo $prop_name; echo $obspan; 	echo $prop_value; ?></td>
	...
	if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS']))
	{
		...
		if ($arItem['CAN_BUY'])
		{
			...
			{
			?>
		<div class="bx_catalog_item_controls_blockone"><div style="display: none;position: relative;">
		<div class="colvo">Количество:&nbsp;&nbsp;&nbsp;</div>
			<div class="kvbr"><a id="<? echo $arItemIDs['QUANTITY_DOWN']; ?>" href="javascript:void(0)" class="bx_bt_button_type_2 bx_small" rel="nofollow">-</a></div>
			<div class="kvcol"><input type="text" class="bx_col_input" id="<? echo $arItemIDs['QUANTITY']; ?>" name="<? echo $arParams["PRODUCT_QUANTITY_VARIABLE"]; ?>" value="<? echo $arItem['CATALOG_MEASURE_RATIO']; ?>" size="10" maxlength="2"
></div>
			<div class="kvbr"><a id="<? echo $arItemIDs['QUANTITY_UP']; ?>" href="javascript:void(0)" class="bx_bt_button_type_2 bx_small" rel="nofollow">+</a></div>
			<span id="<? echo $arItemIDs['QUANTITY_MEASURE']; ?>">&nbsp;&nbsp;&nbsp;штук</span>
		</div></div>
			<?
			}
			?>
		<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="bx_catalog_item_controls_blocktwo">
			<a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="bx_bt_button bx_medium" href="javascript:void(0)" rel="nofollow"><i class="fa fa-shopping-cart"></i></a>
		</div>
			<?
			if ($arParams['DISPLAY_COMPARE'])
			{
				...
				<div class="bx_catalog_item_controls_blocktwo">
					<a id="<? echo $arItemIDs['COMPARE_LINK']; ?>" class="bx_bt_button_type_2 bx_medium" href="javascript:void(0)"><i class="fa fa-shopping-cart"></i></a>
				</div><?
			}
		}
		else
...
var <? echo $strObName; ?> = new JCCatalogSection(<? echo CUtil::PhpToJSObject($arJSParams, false, true); ?>);
	...
	else
	{
		...
		<div class="bx_catalog_item_controls no_touch">
			...
			{
			?>
		<div class="bx_catalog_item_controls_blockone">Количество:
