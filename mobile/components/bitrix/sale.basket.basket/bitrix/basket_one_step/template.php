...
<div class="card-iarga">
	...
	<div class="total-pay_ia">
	
	<span><?=GetMessage('TOTAL_PAY')?> 
	<?if (floatval($arResult["ITEMS"]["AnDelCanBuy"][0]["DISCOUNT_PRICE_PERCENT"]) > 0):?> со скидкой <?=$arResult["ITEMS"]["AnDelCanBuy"][0]["DISCOUNT_PRICE_PERCENT"]?>% <?endif;?>:</span>
	<strong></strong> <i class="sep_ia"></i></div><!--.total-pay-end-->
	...
	<div class="card_ia">    
		...
		//_c($item);
			...
			<div class="item_ia">
				...
				<?else:?>
					<div class="img_ia">			
						<span class="preview_ia"><img src="/img/no_photo.png" alt=""></span>
					</div><!--.img-end-->
				<?endif;?>	
				<div class="summary_ia">
					...
					<div class="description-preview_ia">
						...
						<p class="price_ia">
						<?if (floatval($item["DISCOUNT_PRICE_PERCENT"]) > 0):?>
						<span style="font-size:80%">Цена со скидкой <?=$item["DISCOUNT_PRICE_PERCENT"]?>%:</span> <?=iarga::prep($item['PRICE'])?> <?=GetMessage('VALUTE_MEDIUM')?></p>
									<div class="old_price" id="old_price_<?=$item["ID"]?>">
									Старая цена: <span style="text-decoration:line-through"><?=$item["FULL_PRICE_FORMATED"]?></span>
												<? else: ?>
							<?=iarga::prep($item['PRICE'])?> <?=GetMessage('VALUTE_MEDIUM')?></p>					
											<?endif;?>
	...
	<a name="order_now"></a>
		<form action="<?=$templateFolder?>/order.php" class="uniform_ia autosave_ia" data-rel="order">
			...
			<div class="total-container_ia">
				<a href="#submit" class="bt_ia confirm-button submit_ia" ><?=GetMessage('CONFIRM')?> <img src="<?=$templateFolder?>/images/icon-confirm.png" alt=""></a>
				<p class="error do_order_ia"></p>
				<div class="total-pay_ia"><span><?=GetMessage('TOTAL_PAY')?>
<?if (floatval($arResult["ITEMS"]["AnDelCanBuy"][0]["DISCOUNT_PRICE_PERCENT"]) > 0):?> со скидкой <?=$arResult["ITEMS"]["AnDelCanBuy"][0]["DISCOUNT_PRICE_PERCENT"]?>% <?endif;?>:				
				</span> <strong></strong></div><!--.total-pay-end-->
			</div>
		</form>
	
	<div class="nocard_ia">
		<?=($_SERVER['SERVER_NAME']=="www.test.auto-open.ru")?GetMessage("EMPTY_BASKET_NACH"):GetMessage("EMPTY_BASKET")?>	
	</div>
