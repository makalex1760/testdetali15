...
<div class="dleft">
    <table class="big-basket-fields" width="50%">
<tr class="field">
...
<input  type="radio" <?=$private?> name="type" value="private" id="private"><label for="private">Частное лицо</label><br>
<input  type="radio" <?=$company?>  name="type" value="company" id="company"><label for="company">Представитель юридического лица</label></td>
</tr>
            ...
            <td class="right">
                <input class="ntSaveForms" type='text' size="40" name='PROPERTY[COMPANY_NAME]' value='<? if($admp==0) echo $arUser["WORK_COMPANY"];?>' />            </td>
        </tr>
            ...
            <td class="right">
                <input class="ntSaveForms" type='text' size="40" name='PROPERTY[KONT_NAME]' value='<?=$_SESSION["namekl"]?>' />  <?$_SESSION["namekl"]=$_GET["namekl"];?>          </td>
        </tr>
                <tr class="field">
            <td class="left">
                Контактный телефон:
            ...
            <td class="right">
                <input class="ntSaveForms" type='text' size="40" name='PROPERTY[PHONE]' value='<? if($admp==0) echo $arUser["PERSONAL_PHONE"];?>' id="property_phone" />            </td>
        </tr>
<tr class="field" padding='0px'>
            <td class="left">
                Уведомлять по смс:
            </td>
            <td class="right">
			<? if($admp) $smsdat="Уточни телефон для СМС"; ?> 
                <input class="<?=$tooltip?>" data-title="<?=$smsdat?>" type='checkbox' size="40" name='PROPERTY[sms]' value='Да'  <?=$checksms?>/>  
		...
		Контактный телефон (дополнительный)
            ...
            <td class="right">
                <input class="ntSaveForms"  type='text' size="40" name='PROPERTY[PHONE2]' />           </td>
        </tr>
		<?endif;?>
 ...
 <? if($admp): ?> Предложи сбросить компред на email. Оформить в 1-с счет и отправить на email<?else:?>  Адрес, комментарии и примечания к заказу:<?endif;?>  
            ...
            <td class="right">
                <textarea class="ntSaveForms"  cols="41" rows="5" name='PROPERTY[ABOUT]'></textarea>            </td>
        </tr>
		</table>
</div>
<div class="dleft2">		
		  <table class="big-basket-fields" width="50%">
            ...
            <td class="right">
                <input class="ntSaveForms" type='radio' checked='checked' name='PROPERTY[DELIVERY]' value='6' id="samovivoz" />Самовывоз<br/>
				<input type='radio' name='PROPERTY[DELIVERY]' value='7' id="dost1" />Доставка по Москве и Московской области<br/>
				 <? if($admp) $dost="Предложи наши транспортные компании или уточни пожелания покупателя";?>  
				
				<input type='radio' class="<?=$tooltip?>" data-title="<?=$dost?>" name='PROPERTY[DELIVERY]' value='199' id="dost2" />Доставка по России<br/>            </td>
        </tr>
                <tr class="field">
            <td class="left">
                Способ оплаты:
            ...
            <td class="right">
				...
				<div id="beznal" <? if($company) echo"style='display:block'"; else echo"style='display:none'";?> ><input  type='radio' name='PROPERTY[PAYMENT]' value='14' />Безналичный расчет</div>
				<? if($admp): ?>
				<div id="card" class="<?=$tooltip?>" <? if($private) echo"style='display:block'"; else echo"style='display:none'";?> data-title="В нашем магазине Вы можете оплатить товар картами VISA, MasterCard, Мир"><input  type='radio' name='PROPERTY[PAYMENT]' value='560' />Оплата картой</div>
				<?endif;?> 
				</td>
        </tr>
<? if($admp): ?>	
		...
		Адрес доставки
            ...
            <td class="right">
                <textarea class="ntSaveForms"  cols="41" rows="5" name='PROPERTY[PRIM]'></textarea>            </td>
        </tr>
		<?endif;?>
<?php
...
<td  class="left" id="nodisplay" >Введите телефон или email</td>           
</tr>
