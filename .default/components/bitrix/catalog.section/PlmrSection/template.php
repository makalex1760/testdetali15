...
<div class="catalog-section">
	...
	<table class="katalog" cellspacing="0" cellpadding="0"  width="100%">
		<thead style="background-color:#E2EEFC" >
			<tr >
				<th class="foto hidden-xs" ></th>
				<th><?=GetMessage("CATALOG_TITLE")?></th>
				<?if(count($arResult["ITEMS"]) > 0):
				foreach($arResult["ITEMS"][0]["DISPLAY_PROPERTIES"] as $arProperty):?>
				
				<?if($arProperty["CODE"]=="PRICE_BASE" || !isset($arProperty["NAME"]) || ($arProperty["CODE"]=="OSTATOK" && $admp==0)) continue;?>
				<?if($arProperty["CODE"]!="NALICHIE") $hid="hidden-xs"; else $hid=""; ?> 
				<th class="prop <?=$hid?>"><?=$arProperty["NAME"]?></th>
				<?endforeach;
				endif;?>
		...
		?>
			...
			<tr id="<?=$this->GetEditAreaId($arElement['ID']);?>">
				...
				<td class="foto hidden-xs" width="30px" align="center" style="zoom:1">
...
<span class="tooltip" data-title="<?echo $tooltip_text;?>"><?=$arProperty["DISPLAY_VALUE"]?></span>
				...
				<td class="prop hidden-xs" align="left">
