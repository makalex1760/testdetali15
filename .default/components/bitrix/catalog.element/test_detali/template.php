...
<? } ?><br/>
			...
			<?if($arParams["USE_PRODUCT_QUANTITY"] || count($arResult["PRODUCT_PROPERTIES"])):?>
				...
				<?if($arParams["USE_PRODUCT_QUANTITY"]):?>
					<div style="margin:10px 0px;" valign="center">
...
<div style="float:left; margin:10px">
<input type="hidden" name="price" value="<?=$price?>" id="product_price<?=$arResult["ID"];?>">
...
<a class="c" href="/add2basket2.php?quantity=1&PRODUCT_ID=<?=$arResult["ID"]?>&price=<?=$price?>">Оформить заказ </a>
				...
				<?foreach($arResult["PRODUCT_PROPERTIES"] as $pid => $product_property):?>
					...
					<div valign="top">
						...
						<div><?echo $arResult["PROPERTIES"][$pid]["NAME"]?>:</div>
						<div>
						<?if(
...
<!--				</form>-->
	...
	<?php //var_dump($arResult["PROPERTIES"]["NALICHIE"]); ?>
...
<input type="submit" name="<?echo $arParams["ACTION_VARIABLE"]."ADD2BASKET"?>" value="<?echo GetMessage("CT_BCE_CATALOG_ADD")?>">
	...
	<?endif?>
				...
				<?if($arProperty["CODE"] == "PRICE_BASE" || ($arProperty["CODE"] == "OSTATOK"&& $admp==0) ) continue; ?>
					...
					<?=$arProperty["NAME"]?>:<span class="prop2 <? if($arProperty["CODE"]=='NALICHIE') echo "tooltip";?>" <? if($arProperty["CODE"]=='NALICHIE'){
						switch(trim($arProperty["VALUE"])){
						case 'Нет': echo("style='font-weight:bold; color:red;display: inline'"); $tooltip_text="data-title='Нет в наличии'"; break;
						case 'Есть': echo ("style='font-weight:bold; color:green;display: inline'"); $tooltip_text="data-title='ул. Коминтерна д. 20/2'"; break;
						case 'На складе': echo "style='color:green;display: inline'"; $tooltip_text="data-title='Доставка в течении 1-го рабочего дня'"; break;
						case "Под заказ":$tooltip_text="Сроки и цену уточняйте у менеджера";break;
						default: echo "style='display: inline'";break;
					}; } else { echo "style='display: inline'"; $tooltip_text="";}
					?>  <?echo $tooltip_text;?>>&nbsp;<b><?	echo $arProperty["DISPLAY_VALUE"];?></b>
					</span><br />
				<?endforeach?>
