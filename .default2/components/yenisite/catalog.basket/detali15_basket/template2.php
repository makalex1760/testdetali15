...
<form method="POST" id="order_form">
    <table class="big-basket">
        <tr>
		<th class="head"><?=GetMessage("PHOTO");?></th>
		<th class="head ch"><?=GetMessage("NAME");?></th>
		<th class="head"><?=GetMessage("COUNT");?></th>
		<th class="head ch"><?=GetMessage("PRICE");?></td>
...
<?=$arParams['UE'];?>
        ...
        <?endforeach?>
			<tr><td></td><td></td> <td></td><td colspan="2"><br/><b><?=GetMessage("ITOG");?> <?=$scidka?>:
<?php 
...
<input  type="radio" <?=$company?>  name="type" value="company" id="company"><label for="company">&nbsp;Представитель юридического лица</label></td>
            ...
            <td class="right">
                <input class="ntSaveForms" type='text' size="38" name='PROPERTY[COMPANY_NAME]' value='<? if($admp==0) echo $arUser["WORK_COMPANY"];?>' />            </td>
        </tr>
            ...
            <td class="right">
                <input class="ntSaveForms" type='text' size="38" name='PROPERTY[KONT_NAME]' value='<?=$_SESSION["namekl"]?>' />  <?$_SESSION["namekl"]=$_GET["namekl"];?>          </td>
        </tr>
                <tr class="field">
            <td class="left">
                Контактный телефон:
            ...
            <td class="right">
                <input  class="ntSaveForms" type='text' size="38" name='PROPERTY[PHONE]' value='<? if($admp==0) echo $arUser["PERSONAL_PHONE"];?>' id="property_phone" />            </td>
        </tr>
		<? if($admp): ?>	
		<tr class="field">
            <td class="left">
		Контактный телефон2
            ...
            <td class="right">
                <input class="ntSaveForms"  type='text' size="38" name='PROPERTY[PHONE2]' id="phone" />           </td>
        </tr>
		<?endif;?>
		...
		<tr class="field">
			...
			<? if($admp) $smsdat="Уточни телефон для СМС"; ?> 
                <input class="<?=$tooltip?>" data-title="<?=$smsdat?>" type='checkbox' size="38" name='PROPERTY[sms]' value='Да'  <?=$checksms?>/>  
		...
		<? endif; ?>		
           ...
           <? if($admp) echo "E-mail клиента:"; else  echo "E-mail:"; ?>
            ...
            <td class="right">
                <input class="ntSaveForms" type='text' size="38" name='PROPERTY[EMAIL]' value='<?if($admp==0)echo $arUser["EMAIL"];?>' id="property_email"/>  
 ...
 <? if($admp): ?> Предложи сбросить компред на email. Оформить в 1-с счет и отправить на email<?else:?>  Адрес, комментарии и примечания к заказу:<?endif;?>  
            ...
            <td class="right">
                <textarea class="ntSaveForms"  cols="38" rows="5" name='PROPERTY[ABOUT]'></textarea>            </td>
        </tr>
            ...
            <td class="right">
                <div id="nal"><input  type='radio' checked='checked' name='PROPERTY[PAYMENT]' value='13' /><label>&nbsp;Наличные</label></div>
				<div id="beznal"><input  type='radio' name='PROPERTY[PAYMENT]' value='14' /><label>&nbsp;Безналичный расчет</label></div>
				<? if($admp): ?>
